...
class DetLibraryCommissionSpider(CityScrapersSpider):
    ...
    def start_urls(self):
        ...
        for months_delta in range(-2, 3):
            ...
            urls.append(
                "https://detroitpubliclibrary.org/events/commission/{}".format(
                    month_str
                )
            )
        return urls
    ...
    def _parse_end(self, response, start):
        ...
        duration_str = response.css(
            'tr:nth-child(2) th[scope="row"] + td::text'
        ).extract_first()
    ...
    def _parse_location(response):
        ...
        loc_name_str = response.css(
            'tr:nth-child(3) th[scope="row"] + td a::text'
        ).extract_first()
        return {
            "name": loc_name_str,
            "address": addr_str,
